...
function buildPrompt(selection: {
  ...
  return [
    ...
    selection.connectedLabels?.length
      ? `Leads to or connects with: ${selection.connectedLabels.slice(0, 5).join(", ")}`
      : "",
    "Instructions:",
...
function buildGraphPrompt(prompt: string) {
  return [
    ...
    "6. If the topic is medicine, include PCB and concrete branches like MBBS, BDS, BAMS, BHMS, BUMS, Nursing, Pharmacy, and Paramedical when relevant.",
    "7. If the topic is finance, technology, commerce, arts, law, design, or civil services, expand into actual study paths, exams, and career options.",
    "8. Prefer clear parent-child progression: stream -> foundation -> path -> career outcome.",
    "9. Use concise labels and useful descriptions.",
    "10. Return raw JSON only. No markdown, no explanation.",
    `User request: ${prompt}`
  ].join("\n");
}

function buildDomainSystemPrompt(domain: "science" | "commerce" | "arts" | null | undefined) {
  if (!domain) {
...
function groqStreamPlugin(groqApiKey?: string): Plugin {
  return {
    name: "groq-stream-proxy",
    configureServer(server: ViteDevServer) {
      server.middlewares.use("/api/groq/generate-graph", async (req: any, res: any) => {
        try {
          if (req.method !== "POST") {
            res.statusCode = 405;
          ...
          if (!groqApiKey) {
            res.statusCode = 500;
            res.end("GROQ_API_KEY is not configured on the server.");
            return;
          }
                ...
                messages: [
                  {
                    role: "system",
                    content:
                      "You generate valid JSON roadmaps for interactive learning graphs. Output only JSON and keep ids concise."
                  },
                  {
                    role: "user",
                    content: buildGraphPrompt(payload.prompt)
                  }
                ]
              })
            }
          ...
          if (!groqResponse.ok) {
            ...
            res.end(errorText || "Graph generation request failed.");
            return;
          }
          ...
          res.end(
            error instanceof Error
              ? `Graph generation failed: ${error.message}`
              : "Graph generation failed."
          );
        }
      });

      server.middlewares.use("/api/groq/stream", async (req: any, res: any) => {
        if (req.method !== "POST") {
          res.statusCode = 405;
          res.end("Method Not Allowed");
          return;
        ...
        const payload = JSON.parse(body) as
          | {
              mode?: "selection";
              selection: Parameters<typeof buildPrompt>[0];
            }
          | {
              mode: "chat";
              domain: "science" | "commerce" | "arts" | null;
        ...
        const messages =
          payload.mode === "chat"
            ? [
                {
                  role: "system",
                  content: buildDomainSystemPrompt(payload.domain)
                },
            ...
            : [
                {
                  role: "system",
                  content:
                    "You are a friendly, expert Indian academic career counsellor. Your goal is to guide a student who has just clicked on a node in their career roadmap. Give specific, actionable guidance. Be encouraging, concrete, and focused on real outcomes. Avoid generic advice. Always mention India-specific exams, institutes, or salary ranges where relevant."
                },
                {
                  role: "user",
                  content: buildPrompt(payload.selection)
                }
              ];
    ...
    configurePreviewServer(server: PreviewServer) {
      server.middlewares.use("/api/groq/generate-graph", (_req: any, res: any) => {
        res.statusCode = 501;
        res.end("Preview mode does not expose the graph-generation proxy. Run the dev server or deploy this route on your backend.");
      });
      server.middlewares.use("/api/groq/stream", (_req: any, res: any) => {
        res.statusCode = 501;
        res.end("Preview mode does not expose the Groq proxy. Run the dev server or deploy this route on your backend.");
      });
    }
  };
}
...
export default defineConfig(({ mode }) => {
  ...
  return {
    plugins: [react(), groqStreamPlugin(env.GROQ_API_KEY)],
    server: {
      port: 5173
    }
  };
